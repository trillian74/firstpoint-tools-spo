...
/***/ function(module, exports, __webpack_require__) {
	...
	var _stringify = __webpack_require__(380);
	...
	var _createClass2 = __webpack_require__(399);
	...
	$(function () {
	    ...
	    $(".people-search").change(function () {
	        ...
	        $("input:blank:not(:hidden)").each(function () {
	    ...
	    function ShowAddUser(id) {
	        ...
	        if (e.preventDefault) e.preventDefault();else e.returnValue = false; //IE<9
	    }
	    ...
	    function MoveUp(element) {
	        var input = element.closest(".ms-Grid-row").prev().find("input");

	        if (input.length >= 1) {
	            input.focus().select();
	        } else {
	            input = element.closest(".ms-Grid-row").prev().prev().find("input");
	    ...
	    function MoveDown(element) {
	        var input = element.closest(".ms-Grid-row").next().find("input");

	        if (input.length >= 1) {
	            input.focus().select();
	        } else {
	            input = element.closest(".ms-Grid-row").next().next().find("input");
	    ...
	    function UserSelectMove(direction, element) {
	        ...
	        if (selectedUser.length === 0) {
	            if (direction === "down") {
	                userContainer.find(".user-item:first").addClass("user-item-selected");
	            } else {
	                userContainer.find(".user-item:last").addClass("user-item-selected");
	            }
	        } else {
	            if (direction === "down") {
	    ...
	    var AddEditOnTables = function AddEditOnTables(model) {
	        $("#items > tbody > tr > td:not(.no-row-click)").click(function () {
	            if ($(this).hasClass("ms-Table-rowCheck")) {
	                return;
	            }

	            var id = $(this).parent().prop("id").replace("itemId_", "");
	            window.location.href = "/" + model + "/Edit/" + id;
	        });
	    };

	    var AddCustomEditOnTables = function AddCustomEditOnTables(editFunction) {
	        $("#items > tbody > tr > td:not(.no-row-click)").click(function () {
	            if ($(this).hasClass("ms-Table-rowCheck")) {
	                return;
	            }
	    ...
	    var GetQueryStringParameter = function GetQueryStringParameter(paramToRetrieve) {
	        try {
	    ...
	    var stripTableContentAndAppendToTable = function stripTableContentAndAppendToTable(html) {
	        var tableRows = $(html).find("td").parent();
	    ...
	    var ValidateDatePickerInput = function ValidateDatePickerInput(element) {
	    ...
	    var FormatNumericInput = function FormatNumericInput(num) {
	...
	var BBTOOLSMAN = function () {
	    function BBTOOLSMAN() {
	        (0, _classCallCheck3.default)(this, BBTOOLSMAN);
	    ...
	    (0, _createClass3.default)(BBTOOLSMAN, [{
	        key: "ShowAddUser",
	        value: function ShowAddUser(id) {
	            tools.currentId = id;
	        ...
	        key: "removePeoplePickerValue",
	        value: function removePeoplePickerValue(element) {
	            element.hide();

	            // Reset values
	            element.parent().find("input[type='text']").show().focus().select();
	            element.parent().parent().find("input[type='hidden']").val("");
	        }
	    }, {
	        key: "setPeoplePickerValue",
	        value: function setPeoplePickerValue(picker, displayName, userPrincipalName) {
	            // Set displayName
	            picker.find(".selected-user-display").html("<i class='ms-Icon ms-Icon--x' aria-hidden='true'></i>" + displayName).show();

	            // Set Values
	            picker.find(".display-name").val(displayName);
	            picker.find(".user-principal").val(userPrincipalName);

	            // Hide people search
	            picker.find("input[type='text']").hide();
	        ...
	        key: "selectUser",
	        value: function selectUser(element) {
	            var container = element.parent().parent();
	            var displayName = element.text().trim();
	            var userPrincipal = element.find("input").val();

	            this.setPeoplePickerValue(container, displayName, userPrincipal);
	        }
	    }, {
	        key: "searchForUsers",
	        value: function searchForUsers(element, ctx) {
	            var dao = new SPScript.RestDao();

	            dao.search.people(element[0].value).then(function (result) {
	                $(".user-results-container").remove();
	        ...
	        key: "addLocation",
	        value: function addLocation(item) {
	            var index = this.locations.findIndex(function (o) {
	                // if(o.Id === item.params.data.Id)
	                return o.Id === item.id;
	            }) || -1;
	            index === -1 ? this.locations.push({ Id: item.id, Title: item.title }) : this.locations.splice(index, 1);
	        ...
	        key: "removeLocation",
	        value: function removeLocation(item) {
	            var index = this.locations.findIndex(function (o, i) {
	                // console.log(i);
	                return o.Id === item.id;
	            });
	            this.deleted = this.locations.splice(index, 1);
	            // this     .locations     .map(function(obj) {         console.log('locations['
	            // + obj.Id + '] = ' + obj.Title);     });
	        ...
	        key: "GetUserId",
	        value: function GetUserId(accountName) {
	            var that = this;
	            return new _promise2.default(function (resolve, reject) {
	                ...
	                $.ajax({
	                    ...
	                    success: function success(data) {
	                        ...
	                        console.log(this);
	                        console.log(that);
	                        resolve((0, _stringify2.default)(data.d.Id), that);
	                    },
	                    error: function error(data) {
	                        console.log((0, _stringify2.default)(data));
	                        reject((0, _stringify2.default)(data), that);
	                    }
	                });
	            }.bind(this));
	        }
	        ...
	        value: function assignUserTask() {
	            ...
	            if (filteredInputs.length !== 0) {
	                ...
	                var user = this.GetUserId($(".user-principal").attr("value")).then(function (id, tools) {
	                    console.log("goahead create task");
	                    var dao = new SPScript.RestDao();
	                    if (this.Sites.length === 0) {
	                        dao.web.subsites().then(function (sites) {
	                            this.Sites = sites;
	                            ...
	                            var daoUri = _spPageContextInfo.webAbsoluteUrl.substring(0, _spPageContextInfo.webAbsoluteUrl.indexOf(_spPageContextInfo.webServerRelativeUrl)) + sitedetails[0].ServerRelativeUrl;
	                            var dao = new SPScript.RestDao(daoUri);
	                            var list = dao.lists("Oppgaver");
	                            list.addItem(newItem).then(function (item) {
	                                console.log(item);
	                                item.Title === undefined ? alert("Noe gikk galt, prøv igjen eller kontakt IT.") : this.removeInstallationRow($("." + this.currentId));
	                                CloseDialog();
	                            }.bind(this));
	                        }.bind(this));
	                    } else {
	        ...
	        key: "removeInstallationRow",
	        value: function removeInstallationRow(title) {
	            $(title).remove();
	        }
	    }, {
	        key: "addInstallationRow",
	        value: function addInstallationRow(item) {
	            return "<div class=\"ms-Table-row " + item.Id + "\"><span class=\"ms-Table-cell ms-font-xl\">" + item.Title + "</span><span class=\"ms-Table-cell ms-font-xl\">" + (item.AssignedTo === null ? "<button type=\"button\" class=\"ms-Button ms-Button--command\" id=\"" + item.Id + "\"><span class=\"ms-Button-icon\"><i class=\"ms-Icon ms-Icon--plus\"></i></span>" + " <span class=\"ms-Button-label\">Tilordnet til</span></button>" : item.AssignedTo) + "</span><span class=\"ms-Table-cell ms-font-xl\">" + item.Title + "</span></div>";
	        }
	    }, {
	        key: "consoleMe",
	        value: function consoleMe() {
	            console.log(this);
	        }
	    }]);
	    return BBTOOLSMAN;
	...
	var Civilian = function () {
	    function Civilian(name) {
	        (0, _classCallCheck3.default)(this, Civilian);

	        this.name = name;
	    }

	    (0, _createClass3.default)(Civilian, [{
	        key: "danger",
	...
	var SuperHero = function (_Civilian) {
	    (0, _inherits3.default)(SuperHero, _Civilian);

	    function SuperHero(name, ability) {
	        (0, _classCallCheck3.default)(this, SuperHero);

	        // Call the super class constructor
	        var _this = (0, _possibleConstructorReturn3.default)(this, (SuperHero.__proto__ || (0, _getPrototypeOf2.default)(SuperHero)).call(this, name));

	        _this.ability = ability;
	        return _this;
	    }

	    (0, _createClass3.default)(SuperHero, [{
	        key: "danger",
	        value: function danger() {
	...
	var InitDatePicker = function InitDatePicker(dp) {
	    dp.DatePicker({
	        firstDay: 1,
	        format: "dd.mm.yyyy",
	...
	var AddInstallations = function AddInstallations(tools) {
	    $.ajax({
	        url: webAbsoluteUrl + "/_api/web/getsubwebsfilteredforcurrentuser(nwebtemplatefilter=-1,nconfigurationf" + "ilter=0)?$Select=Id,Title",
	        type: "GET",
	        ...
	        success: function success(data) {
	            ...
	            $.when.apply(null, m).done(function () {
	                $('#tool-installation-picker').on('click', 'input', function () {
	                    // console.log(this);
	                    if (this.checked) {
	...
	var CreateInstallationCheckBox = function CreateInstallationCheckBox(item, i) {
	    return "<div class=\"ms-Grid-col ms-u-sm6 ms-u-md4 ms-u-lg2\"><div class=\"ms-CheckBox\"" + "><input tabindex=\"-1\" title=\"" + item.Title + "\" id=\"" + item.Id + "\"  type=\"checkbox\" class=\"ms-CheckBox-input\"><label role=\"checkbox\" class" + "=\"ms-CheckBox-field\" tabindex=\"" + i + "\" aria-checked=\"false\" name=\"" + item.Title + "\"><span class=\"ms-Label ms-fontSize-l\">" + item.Title + "</span></label></div></div>";
	};
	...
	var RemovePeoplePickerValue = function RemovePeoplePickerValue(element) {
	    element.hide();
...
/***/ function(module, exports, __webpack_require__) {
	...
	// 25.1.2.1.1 %IteratorPrototype%[@@iterator]()
	__webpack_require__(315)(IteratorPrototype, __webpack_require__(350)('iterator'), function(){ return this; });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _setPrototypeOf = __webpack_require__(373);
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  set: Object.setPrototypeOf || ('__proto__' in {} ? // eslint-disable-line
	    function(test, buggy, set){
	      try {
	        set = __webpack_require__(313)(Function.call, __webpack_require__(368).f(Object.prototype, '__proto__').set, 2);
	        set(test, []);
	        buggy = !(test instanceof Array);
	      } catch(e){ buggy = true; }
	      return function setPrototypeOf(O, proto){
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';
	var LIBRARY            = __webpack_require__(333)
	  , global             = __webpack_require__(308)
	  , ctx                = __webpack_require__(313)
	  , classof            = __webpack_require__(385)
	  , $export            = __webpack_require__(311)
	  , isObject           = __webpack_require__(318)
	  , aFunction          = __webpack_require__(314)
	  , anInstance         = __webpack_require__(386)
	  , forOf              = __webpack_require__(387)
	  , speciesConstructor = __webpack_require__(391)
	  , task               = __webpack_require__(392).set
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(!setTask || !clearTask){
	  ...
	  // Node.js 0.8-
	  if(__webpack_require__(343)(process) == 'process'){
	    defer = function(id){
	      process.nextTick(ctx(run, id, 1));
	    };
	  // Browsers with MessageChannel, includes WebWorkers
...
/***/ function(module, exports, __webpack_require__) {
	...
	var global    = __webpack_require__(308)
	  , macrotask = __webpack_require__(392).set
	  , Observer  = global.MutationObserver || global.WebKitMutationObserver
	  , process   = global.process
	  , Promise   = global.Promise
	  , isNode    = __webpack_require__(343)(process) == 'process';
